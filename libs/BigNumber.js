...
function divRemTo(m, q, r) {
    ...
    var nsh = 28 - this.nbits(pm[pm.t - 1]);	// normalize modulus
    if (nsh > 0) {
        pm.lShiftTo(nsh, y);
        pt.lShiftTo(nsh, r);
    } else {
...
function invDigit() {
    ...
    y = (y * (2 - x * y % 268435456)) % 268435456;		// y == 1/x mod 2^dbits
